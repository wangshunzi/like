...
const ServerSlideVertify: React.FC<ServerSlideVertifyProps> = ({showInfo,  onEnd, onRefresh}) => {
    const { originImg, blockBgImg, blockImg, message='滑动验证',fillDefaultColor, fillSuccessColor, fillFailedColor} = showInfo || {}
    ...
    const handleDragEnd = useCallback((e) => {
        ...
        // 回调通知
        // 移动距离
        onEnd && onEnd(moveX, moveX / (dragInfoRef.current.width - blockRef.current.offsetWidth), ()=>setStatus('success'), ()=>setStatus('failed'))
    ...
    return <div className="vertify-container"
            ...
            >
                <div className="img-container">
                    <img src={blockBgImg} alt="" className="slide-block-bg"/>
                    <img ref={blockRef} src={blockImg} alt="" className="slide-block" onLoad={() => {
                        squareRef.current.style.width = blockRef.current.offsetWidth+'px'
                    }}/>
                    {originImg && <img ref={bgRef} src={originImg} alt="" className="slide-bg"  onLoad={()=> {
                        dragInfoRef.current.width = bgRef.current.offsetWidth
                    ...
                    {['loading', 'judging'].includes(status) ? <div className={`status ${status}`}></div>: null} 
                    {['loading', 'judging'].includes(status) ? null : <div className="refresh" onClick={refresh}></div>}
                </div>
