...
class ObserveTool {
  ...
  constructor(moreHeight = "0px") {
    this.map = new Map()
    this.observer = new IntersectionObserver(entries => {
      entries.forEach((entry: any) => {
        if (entry.intersectionRatio > 0 && !entry.target.__visible__) {
...
const VirsualList: React.FC<VirsualListProps> = ({ data = [], children, initPageCount = 10, height = "50vh", autoAdjustPageCount = true }) => {
  ...
  const dataRef = useRef<VirsualListCache>({
    placeholdHeight: 1, // 标识dom高度(调试时，可设置高度，看到标记dom)
  ...
  return (
    <div ref={rootRef} style={{ height: height, overflowY: "auto", position: "relative" }}>
      <div ref={contentRef} style={{ position: "absolute", left: 0, top: 0, width: "100%" }}>
        {renderList.map((item: any) => {
          if (item.__tag__) {
            ...
            return <div id={item.__tag__} ref={ref} key={item.__tag__} style={{ height: `${dataRef.current.placeholdHeight}px`, width: '100%', backgroundColor: "transparent", position: 'absolute' }} />
