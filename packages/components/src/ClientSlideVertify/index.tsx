...
export interface ClientSlideVertifyProps  {
    ...
    onEnd?: (uDistance: number, dDistance: number) => {},
    onSuccess?: () => {},
    onFailed?: () => {},
}
 ...
 */
const loadImg = (imgSrc: string,  onload: (i:HTMLImageElement)=>void, onError: (i:HTMLImageElement)=>void) => {
    const img = new Image();
...
const ClientSlideVertify: React.FC<ClientSlideVertifyProps> = ({showInfo,  onSuccess, onFailed, onEnd}) => {
    const {imgW=400, imgH=200, tolerant=20, imgUrl, blockPosition, message='滑动验证',fillDefaultColor, fillSuccessColor, fillFailedColor} = showInfo || {}
    ...
    const initSlidePane = useCallback(() => {
        ...
        let w = imgW, h = imgH
        // 图片源：为空则随机
        let gImgSrc = () => imgUrl ||  `https://picsum.photos/id/${Math.round(Math.random() * 1084)}/${imgW}/${imgH}`
        // 镂空位置：为空则随机(滑块尺寸 45 x 45), 记录正确位置，用于后续判定
        successInfoRef.current = blockPosition || {x: Math.floor(Math.random() * (w - 145) + 100), y:  Math.floor(Math.random() * (h - 45))}
    ...
    const handleDragStart = useCallback((e) => {
        ...
        dragInfoRef.current.originPosition = {x: e.clientX || e.touches[0].clientX, y: e.clientY || e.touches[0].clientY};
    }, [status])
    ...
    const handleDragEnd = useCallback((e) => {
        ...
        // 回调通知
        // 移动距离
        onEnd && onEnd(moveX, successInfoRef.current.x)
        // 判定逻辑
    ...
    return <div className="vertify-container" style={{width: imgW}}
            ...
            >
                <div className="img-container" style={{width: imgW, height: imgH}}>
                    ...
                    {['pending', 'failed'].includes(status) ? <div className="refresh" onClick={initSlidePane}></div>: null}
                </div>
                <div className="slide-bar noselect" unselectable="on">
                    {message}
                    <div className={`fill ${status}`} ref={fillRef} style={fillDefaultColor ? {backgroundColor: fillDefaultColor} : {}}></div>
                    <div 
