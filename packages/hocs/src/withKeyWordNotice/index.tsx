...
const noticeString: (str: string | number, key: string, kvElement: (kv: string)=>any) => any = (str, key, kvElement) => {
    let wordList = (''+str).split(key)
    
    return <>
                {wordList.map((item, idx) => {
                    return idx !== 0 ? (<React.Fragment key={idx}>
...
const noticeWord: (kv: string, elements: any[], kvElement: (kv: string)=>any) => any = (kv, elements, kvElement) => {
    return React.Children.map(elements, child => {
        ...
        if (child.props && child.props.children && child.props.children.length > 0) {
            return React.cloneElement(child, child.props, noticeWord(kv, child.props.children, kvElement))
        }
        return child
    })
}
